...
public class Sender {// Client
	...
	private static void Usage() {
		System.out.println(
				"Arguments in [] are optional. Any arguments used must be entered in the order displayed here.");
		System.out.println("Usage: Sender [-d #.#] [-s ###] [-t ###] -f inputfile reciever_ip reciever_port");
	}
	// Steps to use:
	// javac Sender.java
	// java Sender -f image.png localhost 8080
		...
		while (i < args.length) {
			...
			if (arg.startsWith("-")) {
				...
				case 'd':
					...
					break;
				case 'f':
					// must have at minimum -f and the filename, the receiver address, and the
					// reciever port
	...
	public static void main(String[] args) {
		...
		try (FileInputStream inputStream = new FileInputStream(inputFile);
			...
			while (inputStream.read(buffer) != -1) {
				...
				System.out.format("Packet: %4d  -  Start Byte Offset: %8d  -  End Byte Offset: %8d%n", ++packetCount,
