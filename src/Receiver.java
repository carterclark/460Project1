		...
		try {
			...
			while (true) {
				...
				if (simulateErrorRng == 1) { // corrupted
					makeAndSendInt(1, serverSocket, receivedDatagram.getAddress(),
							receivedDatagram.getPort());
					
				} else if (simulateErrorRng == 2) { // dupe
					makeAndSendInt(previousAck, serverSocket, receivedDatagram.getAddress(),
							receivedDatagram.getPort());
					
				} else { // successful run
					makeAndSendInt((int) endOffset, serverSocket, receivedDatagram.getAddress(),
							receivedDatagram.getPort());
				...
				// send len
				makeAndSendInt(receivedDatagram.getLength(), serverSocket, receivedDatagram.getAddress(),
						receivedDatagram.getPort());
...
//                serverSocket.send(datagramWithPacket);
	...
	private static void makeAndSendShort(short data, DatagramSocket serverSocket, InetAddress inetAddress, int port)
		...
		DatagramPacket datagramWithAck = new DatagramPacket(ByteBuffer.allocate(4).putShort(data).array(),
				ByteBuffer.allocate(4).putShort(data).array().length, inetAddress, port);
		serverSocket.send(datagramWithAck);
	}

	private static int rngErrorGenerator() {
		int random_int = (int) Math.floor(Math.random() * (0 + 50 + 1) + 1);
		return random_int;
	}
